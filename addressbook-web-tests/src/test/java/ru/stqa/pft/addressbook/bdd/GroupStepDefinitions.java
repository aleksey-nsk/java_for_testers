...
// Класс содержащий описание шагов
public class GroupStepDefinitions {
  ...
  public void loadGroups() {
    groups = app.getDbHelper().getGroups();
  }
  ...
  public void verifyGroupCreated() {
    Groups newGroups = app.getDbHelper().getGroups();
    int maxId = newGroups.stream().mapToInt(g -> g.getId()).max().getAsInt();
    assertThat(newGroups, equalTo(groups.withAdded(newGroup.withId(maxId))));
  }
}
